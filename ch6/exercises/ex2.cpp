// ex2.cpp: Четан Ананд
// Описание: Add the ability to use {} as well as () in the program, so that
//     {(4+5)*6} / (3+4) will be a valid expression.

#include "std_lib_facilities.h"

class Token {	      // Token ਕਲਾਸ ਨੰਬਰ ਅਤੇ ਆਪਰੇਟਰ ਪੜ੍ਹਦੀ ਹੈ।
public:
    char kind;        // Token ਦਾ ਟਾਈਪ।
    double value;     // Token ਦੀ ਵੈਲਯੂ।
    Token(char ch)    // char ਤੋਂ ਇੱਕ Token ਬਣਾਓ।
        :kind(ch), value(0) { }
    Token(char ch, double val)     // char ਅਤੇ double ਤੋਂ ਇੱਕ Token ਬਣਾਓ।
        :kind(ch), value(val) { }
};

//------------------------------------------------------------------------------

class Token_stream {  // Token_stream ਦਾ ਮਕਸਦ cin ਤੋਂ Token ਪੜ੍ਹਨਾ ਹੈ।
public:
    Token_stream();   // ਇੱਕ ਕਨਸਟ੍ਰਕਟ ਜੋ ਕਿ buffer ਨੂੰ ਖਾਲੀ ਕਰ ਦਿੰਦਾ ਹੈ ਅਤੇ full ਨੂੰ false ਡਿਕਲੇਅਰ ਕਰਦਾ ਹੈ।
    Token get();      // Token ਪੜ੍ਹਦਾ ਹੈ।
    void putback(Token t);    // Token ਨੂੰ buffer ਵਿਚ ਪਾ ਦਿੰਦਾ ਹੈ।
private:
    bool full;        // full ਦਾ true ਹੋਣਾ ਇਹ ਦਰਸਾਉਂਦਾ ਹੈ ਕਿ Token_stream ਵਿਚ ਇੱਕ Token ਹੈ।
    Token buffer;     // buffer Token ਨੂੰ ਸਟੋਰ ਕਰਦਾ ਹੈ ਜਦੋਂ putback() ਵਰਤਿਆ ਜਾਂਦਾ ਹੈ। 
};

//------------------------------------------------------------------------------

Token_stream::Token_stream()        // Token_stream ਦੀ ਡੇਫੀਨਿਸ਼ਨ
    : full(false), buffer(0) { }    // ਮਤਲਬ ਕਿ buffer ਵਿਚ ਕੋਈ Token ਨਹੀ ਹੈ।

//------------------------------------------------------------------------------

void Token_stream::putback(Token t)     // Token ਨੂੰ buffer ਵਿਚ ਪਾ ਦਿੰਦਾ ਹੈ।
{
    if (full) error("putback() into a full buffer");
    buffer = t;       // t ਨੂੰ buffer ਵਿਚ ਕਾਪੀ ਕਰਦਾ ਹੈ। 
    full = true;      // buffer ਹੁਣ ਭਰਿਆ ਹੋਇਆ ਹੈ।
}
